...
const SellerDashboard = ({
  ...
  const deletePurchaseDraft = async (draft, options = {}) => {
    ...
    const rows = Array.isArray(draft?.rows) ? draft.rows : [];
    if (!draft?.targetSellerId || rows.length === 0) {
      setBookingError('Delete karne ke liye purchase draft nahi mila');
      return;
    }
    ...
    try {
      ...
      setBookingError(error.response?.data?.message || error.message || 'Purchase delete nahi hua');
    } finally {
      if (shouldRefresh) {
        setSendPurchaseSendingKey('');
      }
  ...
  const deleteAllPurchaseDrafts = async () => {
    ...
    try {
      ...
      setBookingError(error.response?.data?.message || error.message || 'All purchase delete nahi hua');
    } finally {
      setSendPurchaseSendingKey('');
    }
  };
  const deleteSellerPurchaseDrafts = async (sellerDraft) => {
    ...
    try {
      ...
      setBookingError(error.response?.data?.message || error.message || 'Seller purchase delete nahi hua');
    } finally {
      setSendPurchaseSendingKey('');
    }
  };
