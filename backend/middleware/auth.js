  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, user) => {
    ...
    try {
      const userResult = await query(
        'SELECT id, username, keyword, role, seller_type, parent_id, owner_admin_id, can_login, rate_amount_6, rate_amount_12 FROM users WHERE id = $1 LIMIT 1',
        [user.id]
      );
      ...
      req.user = {
        ...
        ownerAdminId: currentUser.owner_admin_id,
